...
function ajax_req_handle( $id, $isdinamccntn ) {
	...
	$isgp = easy_get_option( 'easymedia_disen_gp' ); $iseml = easy_get_option( 'easymedia_disen_eml' ); $isfb = easy_get_option( 'easymedia_disen_fb' ); $istwt = easy_get_option( 'easymedia_disen_twt' ); $ispin = easy_get_option( 'easymedia_disen_pin' );
...
if ( $isfb || $istwt || $ispin || $isgp || $iseml ): ?>

              <div id="mbsosmed">
            	<ul>
                <?php if ( $isfb ): ?>
                  <li id="sosmedfb">
					<a onClick="window.open('http://www.facebook.com/sharer.php?s=100&amp;p[title]=<?php echo urlencode( $sharemediattl ); ?>&amp;p[summary]=<?php echo urlencode(strip_tags( $shortenerdesc ) ); ?>&amp;p[url]=CONVTOURL&amp;&amp;p[images][0]=<?php echo urlencode( $imgsrc ); ?>','sharer','toolbar=0,status=0,width=548,height=325');" href="javascript: void(0)"><span title="Share it!"></span></a>
                  </li>
                  <?php endif; ?>
                ...
                <?php if ( $ispin ): ?>                  
                  <li id="sosmedpn">
                  	<a onClick="window.open('http://pinterest.com/pin/create/button/?url=<?php echo urlencode( $boxlink ); ?>&media=<?php echo urlencode( $imgsrc ); ?>&description=<?php echo urlencode(strip_tags( $shortenerdesc ) ); ?>','sharer','toolbar=0,status=0,width=575,height=330');" href="javascript: void(0)"><span title="Pin it!"></span></a>
                  </li>
                  <?php endif; ?>
