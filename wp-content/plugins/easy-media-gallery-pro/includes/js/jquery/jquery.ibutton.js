jQuery(document).ready(function($) {
			...
			jQuery("span.switch").click(function() {
				...
				jQuery(this).next()[0].checked = !jQuery(this).next()[0].checked;
					...
					if (jQuery("#emgtinymce_cat_pag").is(':checked')) {
					jQuery('#mediacatpag').show("slow");
					} else {
					jQuery('#mediacatpag').hide("slow");	
					}
					
					if (jQuery("#emgtinymce_grid_pag").is(':checked')) {
					jQuery('#mediagridpag').show("slow");
					} else {
					jQuery('#mediagridpag').hide("slow");	
					}				
					
					if (jQuery("#easmedia_metabox_media_video_fetch").is(':checked')) {
					jQuery('.easmedia_metabox_img').hide("slow");	
					jQuery('#imgpreviewboxvid').show("slow");	
					jQuery('#fetchthumb').show("slow");	
					jQuery('#customvimyutimgurl').val("");	
					jQuery("#cusimgthumbnailprv").attr('src', 'http://placehold.it/250x150');	
